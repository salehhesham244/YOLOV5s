...
class SPP(nn.Module):
    ...
    def forward(self, x):
        """Applies convolution and average pooling layers to the input tensor `x`, concatenates results, and returns output
        tensor.
        """
        x = self.cv1(x)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress torch 1.9.0 Avg_pool2d() warning
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        """Processes input through a series of convolutions and Avg pooling operations for feature extraction."""
        x = self.cv1(x)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress torch 1.9.0 avg_pool2d() warning
            y1 = self.m(x)
